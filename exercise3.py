...
class Rectangle:
    ...
    def is_in_interval(self, lower: float, upper: float, x: float, tolerance: float = 0.0) -> bool:
        assert tolerance >= 0.0, "Tolernance should be non negative."
        return lower - tolerance <= x and x <= upper + tolerance
    
    def contains(self, point: Point2D, tolerance: float = 0.0) -> bool:
        ll_px = point.x - self._lower_left.x
        ll_py = point.y - self._lower_left.y
        return self.is_in_interval(0, self._dx, ll_px, tolerance) and self.is_in_interval(0, self._dy, ll_py, tolerance)
